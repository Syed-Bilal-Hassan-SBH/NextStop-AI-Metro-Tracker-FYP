...
const MapScreen = () => {
  ...
  const handlePlanJourney = async () => {
    ...
    setJourneyResult(null);
    try {
      const res = await fetch(`${API_URL}/api/journey/quick-plan`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          source: journeyFrom.trim(),
          destination: journeyTo.trim(),
        }),
      });
      const data = await res.json();
      setJourneyResult(data);
    } catch (e) {
      setJourneyResult({success: false, message: 'Could not connect to server'});
    } finally {
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.fabContainer}>
        ...
        <TouchableOpacity
          style={[s.fab, activeModal === 'journey' && s.fabActive]}
            ...
            <View style={s.modalHeader}>
              ...
              <Text style={s.modalTitle}>
                {activeModal === 'list' ? '📋 Bus List' :
                 activeModal === 'journey' ? '🗺️ Journey Planner' :
              ...
              {activeModal === 'journey' && (
                <View>
                  <View style={s.journeyHeader}>
                    <Text style={s.journeySubtitle}>Find the best route between two stops</Text>
                  </View>

                  {/* From */}
                  <Text style={s.label}>📍 From</Text>
                  <TextInput
                    style={s.journeyInput}
                    placeholder="e.g. Khanna Pul Stop"
                    placeholderTextColor="#aaa"
                    value={journeyFrom}
                    onChangeText={setJourneyFrom}
                  />
                  ...
                  <TextInput
                    style={s.journeyInput}
                    placeholder="e.g. NUST/G-12 Metro Station"
                    placeholderTextColor="#aaa"
                    value={journeyTo}
                    onChangeText={setJourneyTo}
                  />

                  <TouchableOpacity
                    style={[s.planBtn, journeyLoading && {opacity: 0.7}]}
                      ...
                      {journeyResult.success === false ? (
                        <View style={s.noResultBox}>
                          ...
                          <Text style={s.noResultTxt}>
                            {journeyResult.message || 'No route found. Try different stop names.'}
                          </Text>
                        </View>
                      ) : (
                        <View>
                          <Text style={s.resultTitle}>✅ Route Found!</Text>

                          {/* Route info */}
                          {journeyResult.route_id && (
                            <View style={s.resultRow}>
                              <Text style={s.resultLabel}>🚌 Route</Text>
                              <Text style={s.resultValue}>
                                {journeyResult.route_name || journeyResult.route_id}
                              </Text>
                            </View>
                          )}

                          {/* From → To */}
                          {(journeyResult.source || journeyResult.destination) && (
                            <View style={s.routePathBox}>
                              <Text style={s.routePathFrom}>
                                📍 {journeyResult.source || journeyFrom}
                              </Text>
                              <Text style={s.routeArrow}>↓</Text>
                              <Text style={s.routePathTo}>
                                🏁 {journeyResult.destination || journeyTo}
                              </Text>
                            </View>
                          )}
                          ...
                          <View style={s.statsRow}>
                            {journeyResult.distance != null && (
                              <View style={s.statChip}>
                                <Text style={s.statEmoji}>📏</Text>
                                <Text style={s.statValue}>
                                  {Number(journeyResult.distance).toFixed(1)} km
                                </Text>
                                <Text style={s.statLabel}>Distance</Text>
                              </View>
                            )}
                            {journeyResult.estimated_time != null && (
                              <View style={s.statChip}>
                                <Text style={s.statEmoji}>⏱️</Text>
                                <Text style={s.statValue}>
                                  {Math.round(journeyResult.estimated_time)} min
                                </Text>
                                <Text style={s.statLabel}>Est. Time</Text>
                              </View>
                            )}
                            {journeyResult.estimated_fare != null && (
                              <View style={s.statChip}>
                                <Text style={s.statEmoji}>💰</Text>
                                <Text style={s.statValue}>
                                  Rs. {journeyResult.estimated_fare}
                                </Text>
                          ...
                          {/* Segments if any */}
                          {journeyResult.segments && journeyResult.segments.length > 0 && (
                            <View style={s.segmentsBox}>
                              <Text style={s.segmentsTitle}>Journey Steps:</Text>
                              {journeyResult.segments.map((seg: any, i: number) => (
                                <View key={i} style={s.segmentRow}>
                                  <Text style={s.segmentNum}>{i + 1}</Text>
                                  <Text style={s.segmentTxt}>
                                    {seg.instruction || seg.description || JSON.stringify(seg)}
                                  </Text>
                                </View>
                              ))}
                            </View>
...
const s = StyleSheet.create({
  ...
  modalContentInner: {
    ...
    height: SCREEN_HEIGHT * 0.75, elevation: 10,
  },
  modalHeader: {
  ...
  // Journey Planner
  journeyHeader: {marginBottom: 16},
  journeySubtitle: {fontSize: 13, color: '#888', textAlign: 'center'},
  ...
  swapBtn: {
    alignSelf: 'center', backgroundColor: '#E3F2FD',
    paddingHorizontal: 16, paddingVertical: 6,
    borderRadius: 20, marginBottom: 8,
  },
  swapTxt: {fontSize: 13, color: '#1565C0', fontWeight: '600'},
  planBtn: {
    backgroundColor: '#1565C0', borderRadius: 25, paddingVertical: 14,
    alignItems: 'center', marginTop: 8, elevation: 3,
  },
  planBtnTxt: {color: '#fff', fontSize: 16, fontWeight: 'bold'},

  resultBox: {marginTop: 16},
  noResultBox: {alignItems: 'center', paddingVertical: 24},
  noResultEmoji: {fontSize: 40, marginBottom: 8},
  noResultTxt: {fontSize: 14, color: '#888', textAlign: 'center'},
  resultTitle: {
  ...
  resultValue: {fontSize: 13, fontWeight: '600', color: '#222', flex: 1, textAlign: 'right'},
  routePathBox: {
    backgroundColor: '#F5F5F5', borderRadius: 10, padding: 14,
    marginVertical: 12, alignItems: 'center',
  },
  routePathFrom: {fontSize: 14, color: '#1565C0', fontWeight: '600'},
  routeArrow: {fontSize: 20, color: '#aaa', marginVertical: 4},
  routePathTo: {fontSize: 14, color: '#E53935', fontWeight: '600'},
  statsRow: {flexDirection: 'row', justifyContent: 'space-around', marginVertical: 12},
  statChip: {
    alignItems: 'center', backgroundColor: '#E3F2FD',
    borderRadius: 10, padding: 12, flex: 1, marginHorizontal: 4,
  },
  statEmoji: {fontSize: 20, marginBottom: 4},
  statValue: {fontSize: 14, fontWeight: 'bold', color: '#1565C0'},
  statLabel: {fontSize: 11, color: '#888', marginTop: 2},
  segmentsBox: {
    backgroundColor: '#F9F9F9', borderRadius: 10, padding: 12, marginTop: 8,
  },
  segmentsTitle: {fontSize: 13, fontWeight: '700', color: '#444', marginBottom: 8},
  segmentRow: {flexDirection: 'row', alignItems: 'flex-start', marginBottom: 6},
  segmentNum: {
    width: 22, height: 22, borderRadius: 11, backgroundColor: '#1565C0',
    color: '#fff', fontSize: 12, textAlign: 'center', lineHeight: 22,
    marginRight: 8, fontWeight: 'bold',
  },
  segmentTxt: {flex: 1, fontSize: 13, color: '#444'},
  ...
  customInput: {
    borderWidth: 1, borderColor: '#ddd', borderRadius: 10,
    paddingHorizontal: 14, paddingVertical: 12,
    fontSize: 15, color: '#222', backgroundColor: '#fafafa', marginBottom: 20,
  },
