...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        # AI chatbot: conversation sessions and messages
        cursor.execute('''
...
# All routes stops data
    ...
    def create_chat_session(
        ...
        cursor.execute(
            '''
            INSERT INTO chat_sessions (id, user_email, title, created_at)
            VALUES (?, ?, ?, ?)
            ''',
            (session_id, user_email, title, created_at),
        )
        conn.commit()
        conn.close()
        return session_id
    ...
    def get_chat_session(self, session_id: str) -> Optional[Dict]:
        """Return session row or None if missing."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(
            'SELECT id, user_email, title, created_at FROM chat_sessions WHERE id = ?',
            (session_id,),
        )
        row = cursor.fetchone()
        conn.close()
    ...
    def get_chat_messages(
        ...
        """Messages ordered oldest first for model context."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(
            '''
    ...
    def append_chat_message(
        ...
        """Append a message; returns new message id."""
        created_at = datetime.now().isoformat()
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        cursor.execute(
